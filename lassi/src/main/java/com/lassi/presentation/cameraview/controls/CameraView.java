...
public class CameraView extends FrameLayout implements LifecycleObserver {
    ...
    private boolean isAllowedAudio = false;
...
//    @OnLifecycleEvent(Lifecycle.Event.ON_RESUME)
     ...
     */
    public void checkPermissionsManifestOrThrow(@NonNull Audio audio) {
        if (audio == Audio.ON) {
            try {
    ...
    public void takePictureSnapshot() {
        ...
        mCameraController.takePictureSnapshot(AspectRatio.Companion.of(getWidth(), getHeight()));
    }
    ...
    public void takeVideoSnapshot(@NonNull File file) {
        if (getWidth() == 0 || getHeight() == 0) return;
        mCameraController.takeVideoSnapshot(file, AspectRatio.Companion.of(getWidth(), getHeight()));
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
                mKeepScreenOn = getKeepScreenOn();
    ...
    public Size getSnapshotSize() {
        ...
        AspectRatio viewRatio = AspectRatio.Companion.of(getWidth(), getHeight());
     ...
     * Defaults to unlimited size. Use 0 or negatives to lassiOption.
     *
     ...
     * Defaults to unlimited duration. Use 0 or negatives to lassiOption.
     *
