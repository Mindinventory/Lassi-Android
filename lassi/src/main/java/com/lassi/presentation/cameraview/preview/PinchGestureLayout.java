...
public class PinchGestureLayout extends GestureLayout {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mNotify) {
            getPoints()[0].x = event.getX(0);
            getPoints()[0].y = event.getY(0);
            if (event.getPointerCount() > 1) {
                getPoints()[1].x = event.getX(1);
                getPoints()[1].y = event.getY(1);
            }
            return true;
        }
        return false;
    ...
    public float scaleValue(float currValue, float minValue, float maxValue) {
        ...
        return GestureLayout.capValue(currValue, currValue + add, minValue, maxValue);
    }
}
